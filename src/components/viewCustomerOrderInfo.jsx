...
export default function ViewCustomerOrderInfo({ order }) {
    ...
    <Modal isOpen={isModalOpen} onRequestClose={()=> setIsModalOpen(false)}
        ...
        <div className="space-y-2 mb-6">
            ...
            <div className="flex items-center gap-3">
                {/* Status Badge */}
                <span className={`px-2 py-1 rounded-md text-white text-xs ${ order.status==="Pending" ? "bg-yellow-500" :
                    order.status==="Completed" ? "bg-green-600" :order.status==="Cancelled" ? "bg-red-600" : "bg-secondary/50" }`}>
                    {order.status}
                </span>
            </div>
            ...
            <textarea value={order.notes?? "No notes provided for this order"} disabled  className="w-full mt-1 p-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-accent 
            focus:outline-none bg-primary/30 text-sm resize-none min-h-[120px]">
            </textarea>
